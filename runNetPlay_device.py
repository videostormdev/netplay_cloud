...
def stream_handler(message):
	...
	if (("put" in message["event"])or("patch" in message["event"])):
		...
		# write to stdout
		if ("commandToSend" in message["path"]):
			if (message["data"]!=last_cmd):
				print(message["data"],flush=True)
				last_cmd = message["data"];
		elif ("SendCnt" in message["path"]):
			#ignore this
			nop=1
		else:
			if (message["data"]["commandToSend"]!=last_cmd):
				print(message["data"]["commandToSend"],flush=True)
				last_cmd = message["data"]["commandToSend"];
...
while loop_var>0:
	...
	if ((user != None) and (my_stream==None) and (db_init<0)):
		...
		try:
			...
			addnew=True
		if (addnew):
			print("new device",file=sys.stderr)
			#create new
			try:
				data = {"connected" : "false", "last_connection" : "-1"}
				db.child("users").child(uid).child("devices").child(uuid).child("DT").set(data,user['idToken'])
				data = {"commandToSend" : "", "SendCnt" : "0"}
				db.child("users").child(uid).child("devices").child(uuid).child("TX").set(data,user['idToken'])
				data = {"commandToRecv" : "", "RecvCnt" : "0"}
				db.child("users").child(uid).child("devices").child(uuid).child("RX").set(data,user['idToken'])
			except:
				print("could not init new device",file=sys.stderr)
				loop_var=0
	...
	if ((my_stream==None)and(loop_var>0)):
		try:
			...
			my_stream = db.child("users").child(uid).child("devices").child(uuid).child("TX").stream(stream_handler,user['idToken'])
		except:
			my_stream = None
	...
	if (loop_var>0):
		...
		if (len(readable)>0):
			...
			rc=0
			try:
				dt = db.child("users").child(uid).child("devices").child(uuid).child("RX").get(user['idToken'])
				#print(str(dt.val()),file=sys.stderr)
				#dt.val = {"commandToRecv" : "val", "RecvCnt" : "int"}
				rc = int(dt.val()["RecvCnt"]) + 1
				#print("Current data",str(dt.val()),file=sys.stderr)
				data2 = {"commandToRecv": data, "RecvCnt": str(rc)}
				print(str(data2),file=sys.stderr)
				db.child("users").child(uid).child("devices").child(uuid).child("RX").update(data2,user['idToken'])
			except:
				#data lost	
